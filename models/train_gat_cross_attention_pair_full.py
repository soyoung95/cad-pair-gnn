...
def clean_graph(graph: Any, default_edge_dim: int = 16) -> Data:
    if not hasattr(graph, "x"):
        raise ValueError("Graph does not have x.")

    if not hasattr(graph, "edge_index"):
        raise ValueError("Graph does not have edge_index.")

    if not hasattr(graph, "edge_attr") or graph.edge_attr is None:
        edge_count = int(graph.edge_index.shape[1])
        graph.edge_attr = torch.zeros((edge_count, default_edge_dim), dtype=torch.float32)
...
def main() -> None:
    ...
    _training_start_time = time.perf_counter()
    _training_started_at = datetime.now()

    def _write_training_time_at_exit():
        _training_elapsed_sec = time.perf_counter() - _training_start_time
        _training_finished_at = datetime.now()
        print(f"TOTAL_TRAINING_TIME_SEC: {_training_elapsed_sec:.2f}")
        print(f"TOTAL_TRAINING_TIME_MIN: {_training_elapsed_sec / 60.0:.2f}")

        try:
            _timing_output_dir = Path(args.output_dir) / f"seed_{args.seed}"
            _timing_output_dir.mkdir(parents=True, exist_ok=True)
            _timing_info = {
                "started_at": _training_started_at.isoformat(timespec="seconds"),
                "finished_at": _training_finished_at.isoformat(timespec="seconds"),
                "elapsed_sec": round(_training_elapsed_sec, 3),
                "elapsed_min": round(_training_elapsed_sec / 60.0, 3),
                "epochs": getattr(args, "epochs", None),
                "batch_size": getattr(args, "batch_size", None),
                "hidden_dim": getattr(args, "hidden_dim", None),
                "num_layers": getattr(args, "num_layers", None),
                "gat_heads": getattr(args, "gat_heads", None),
                "cross_heads": getattr(args, "cross_heads", None),
                "dropout": getattr(args, "dropout", None),
                "lr": getattr(args, "lr", None),
                "weight_decay": getattr(args, "weight_decay", None),
                "graph_dir": getattr(args, "graph_dir", None),
                "pair_index_dir": getattr(args, "pair_index_dir", None),
                "output_dir": getattr(args, "output_dir", None),
                "seed": getattr(args, "seed", None),
            }
            with (_timing_output_dir / "training_time.json").open("w", encoding="utf-8") as f:
                json.dump(_timing_info, f, indent=2, ensure_ascii=False)
            print(f"Training time saved: {_timing_output_dir / 'training_time.json'}")
        except Exception as exc:
            print(f"[WARN] Failed to save training_time.json: {exc}")

    atexit.register(_write_training_time_at_exit)
    set_seed(args.seed)
